...
## Where do I specify the Slack channel in the Destination configuration?
...
## Is it possible to customize the message that is generated from {{site.data.keyword.cloud_notm}} services (({{site.data.keyword.IBM_notm}} Managed sources)?
{: #faq-en-sms-customize}
{: faq}

You cannot customize messages that is generated from {{site.data.keyword.cloud_notm}} services (({{site.data.keyword.IBM_notm}} Managed sources). These notifications are generated by the respective {{site.data.keyword.cloud_notm}} service such as {{site.data.keyword.secrets-manager_short}}, {{site.data.keyword.compliance_short}}, etc. The message content cannot be modified by the end user before it is sent out to a destination.

## Why do I get a 422 error response for the Send Notifications API call?
{: #faq-en-send-notification-error-422}
{: faq}

Error response 422 is "Unprocessable Entity". {{site.data.keyword.en_short}} service is unable to process your request. This is usually seen when there is no condition or filter is associated with the source to which the notification is sent. Check your topic and verify that it is connected to the right source, with the intended conditions.
